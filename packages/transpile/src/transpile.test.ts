...
test('function mappings', () => {
  ...
  expect(scalarOf('ASCII(x)')).toBe('mssqlite_ascii("x")')
  expect(scalarOf('CHAR(x)')).toBe('mssqlite_char("x")')
...
test('cast and convert', () => {
  ...
  expect(scalarOf('CAST(x AS date)'))
    .toBe('mssqlite_temporal_cast("x", \'date\', 0)')
  expect(scalarOf('TRY_CAST(x AS datetime2)'))
    .toBe('mssqlite_temporal_cast("x", \'datetime2\', 1)')
...
test('case, in, like, between, exists', () => {
  ...
  expect(scalarOf('x LIKE \'a%\''))
    .toBe('(mssqlite_collation_like("x", \'a%\', \'sql_latin1_general_cp1_ci_as\', \'\'))')
  expect(scalarOf('x NOT LIKE \'!_%\' ESCAPE \'!\''))
    .toBe('(NOT mssqlite_collation_like("x", \'!_%\', \'sql_latin1_general_cp1_ci_as\', \'!\'))')
